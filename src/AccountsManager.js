...
const LoadingButton = () => (
    <div className="cursor-pointer mb-4">
        <div
            className={"bg-white h-12 w-12 flex items-center justify-center text-black text-2xl font-semibold rounded-lg mb-1 overflow-hidden opacity-25 animate-pulse"}/>
    ...
    return (
        <div onClick={props.callback} className="cursor-pointer mb-4 focus:outline-none">
            <button onClick={props.callback}
                    className={"focus:outline-none bg-white h-12 w-12 flex items-center justify-center text-black text-2xl font-semibold rounded-lg mb-1 overflow-hidden " + (props.addButton ? "opacity-25 " : "") + (props.selected ? "ring" : "")}>
                {props.addButton ? <AddButton/> : (props.img !== false ?
                    <img src={props.img} alt={props.accountName}/> : "N")}
            </button>
...
class AccountsManager extends React.Component {
    ...
    refreshState = (digits = 4) => {
        console.log("Refresh!")
        console.log(this.userState().stateNum)
        this.setState({
            stateNum: genRandomDigits(digits)
        });
    }
    ...
    render() {
        return (
            <div className="h-screen antialiased flex w-full z-0s ">
                <div
                    className="bg-gray-800 text-purple-lighter flex-none w-24 p-6 hidden lg:block overflow-auto scrollbar-thin ">
                    {this.props.state.loggedInUsers === false ? (
                        ...
                        <div>
                            {this.props.state.loggedInUsers.map((user, key) => (
                                <AccountButton key={key} img={user.icon} subTitle={user.display_id}
                                               selected={user.selected} callback={() => this.switchAccount(user.id)}/>
                            ))}
                            <AccountButton addButton={true} callback={() => this.props.accessor({
                                popup: {
                                    title: "usersCreation",
                                    page: "create_user"
                                }
                ...
                <BrowserRouter>
                    ...
                    {this.userState() === undefined ? (
                        <div className="flex-1 flex flex-col dark:bg-gray-800 overflow-auto">
                            <div
                                className="dark:text-white scrollbar-thin scrollbar-thumb-gray-500 hover:scrollbar-thumb-gray-400 overflow-y-scroll">
                            ...
                            <div>
                                <Switch>
                                    <Route exact path="/" render={(props) => <TimeLine
                                        key={`${this.userState()?.userInfo.id}_${props.match.params.id}_${this.userState().stateNum}`}
                                        ...
                                        {...props}
                                    />}/>
                                    <Route path="/p/video" component={VideoPlayer}/>
                                    ...
                                    <Route path="/secret" render={(props) => (
                                        <div className="dark:text-gray-400">
                                            ...
                                            <YouTube videoId="dQw4w9WgXcQ" onReady={(event) => {
                                                setTimeout(() => {
                                    ...
                                    <Route
                                        render={() => <NotFound key={this.userState()?.userInfo.id} state={this.state}
                                                                baseState={this.props.state}/>}/>
                                </Switch>
                                {(this.userState()?.popup?.title === "newPost") ?
                                    <NewPost toggleAccessor={this.toggleAccessor} accessor={this.accessor}
                                             state={this.userState()} baseState={this.props.state}/> : null}
                                {(this.userState()?.popup?.title === "addApp") ?
                                    <NewToLogin toggleAccessor={this.toggleAccessor} accessor={this.accessor}
                                                state={this.userState()} baseState={this.props.state}/> : null}
                            </div>
                            <br/><br/>
                        </div>)}
